    ...
    const connectWs = () => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          // Trigger native notification on 'mention' if running in Electron
          // @ts-ignore
          if (data.type === 'mention' && window.electronAPI) {
            ...
            window.electronAPI.showNotification({
              ...
              channelUrl: `/app/servers/${data.payload?.server_id || 'default'}/channels/${data.payload?.channel_id || 'general'}`
            });
          }
        } catch (e) {}
      };
